<?php
/*******************************************************************************
 ** Common Variables, Constants, and Codes
 *******************************************************************************/
error_reporting(
...
// Set cookies to pass even if security settings or frames are different
header(
...
if (version_compare(PHP_VERSION, '5.2.17', '<')) {
  die(sprintf(
    'PHP 5.2.17 or higher required. Your PHP version is %s', PHP_VERSION
  ));
}

//==========================================================================================================================
// extract($_GET); 명령으로 인해 page.php?_POST[var1]=data1&_POST[var2]=data2 와 같은 코드가 _POST Avoid being used as a variable
// 081029 : letsgolee You have helped..
//--------------------------------------------------------------------------------------------------------------------------
$ext_arr = array('PHP_SELF', '_ENV', '_GET', '_POST', '_FILES', '_SERVER',
                 '_COOKIE', '_SESSION', '_REQUEST',
                 'HTTP_ENV_VARS', 'HTTP_GET_VARS', 'HTTP_POST_VARS',
                 'HTTP_POST_FILES', 'HTTP_SERVER_VARS',
...
function g5_path()
{
  ...
  $server_script_name = preg_replace(
    '/\/+/', '/', str_replace('\\', '/', $_SERVER['SCRIPT_NAME'])
  );
  $server_script_filename = preg_replace(
  ...
  $http = 'http' . ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] == 'on')
      ? 's' : '') . '://';
  $user = str_replace(
    preg_replace($pattern, '', $server_script_filename), '', $server_script_name
  );
...
include_once($g5_path['path'] . '/config.php');   // config file

unset($g5_path);


// multi-dimensional array에 Apply custom function
...
include_once(G5_LIB_PATH
  . '/cache.lib.php');     // cache 함수 및 object cache class 모음
...
if (file_exists($dbconfig_file)) {
  ...
  <div class="ins_inner">
    ...
    <ul>
      <li><strong><?php echo G5_DATA_DIR . '/' . G5_DBCONFIG_FILE ?></strong>
      </li>
    </ul>
...
@ini_set(
  "url_rewriter.tags", ""
); // 링크에 PHPSESSID가 따라다니는것을 무력화함 (해뜰녘님께서 알려주셨습니다.)
  ...
  if (!function_exists('session_start_samesite')) {
    function session_start_samesite($options = array())
    {
      ...
      if (preg_match('/Edge/i', $_SERVER['HTTP_USER_AGENT'])
        ...
        || preg_match(
          '~Trident/7.0(; Touch)?; rv:11.0~', $_SERVER['HTTP_USER_AGENT']
        )
      ...
      foreach ($headers as $header) {
        ...
        $header = preg_replace('~; secure(; HttpOnly)?$~', '', $header)
...
if ($config['cf_editor']) {
  define(
    'G5_EDITOR_LIB', G5_EDITOR_PATH . "/{$config['cf_editor']}/editor.lib.php"
  );
} else {
...
define(
  'G5_CAPTCHA_DIR',
  !empty($config['cf_captcha']) ? $config['cf_captcha'] : 'kcaptcha'
);
...
if (isset($_REQUEST['url'])) {
  ...
  if (G5_DOMAIN) {
    ...
    $urlencode = G5_DOMAIN . urldecode(
        preg_replace("/^" . urlencode($p['path']) . "/", "", $urlencode)
      );
  }
}
...
if ($_SESSION['ss_mb_id']) { // 로그인중이라면
  ...
  if ($member['mb_intercept_date']
    ...
    if (substr($member['mb_today_login'], 0, 10) != G5_TIME_YMD) {
      // 첫 로그인 포인트 지급
      insert_point(
        $member['mb_id'], $config['cf_login_point'], G5_TIME_YMD . ' 첫로그인',
        '@login', $member['mb_id'], G5_TIME_YMD
      );

      // 오늘의 로그인이 될 수도 있으며 마지막 로그인일 수도 있음
      // 해당 회원의 접근일시와 IP 를 저장
      $sql = " update {$g5['member_table']} set mb_today_login = '"
        . G5_TIME_YMDHIS
        . "', mb_login_ip = '{$_SERVER['REMOTE_ADDR']}' where mb_id = '{$member['mb_id']}' ";
      sql_query($sql);
    }
  }
} else {
  ...
  if ($tmp_mb_id = get_cookie('ck_mb_id')) {
    ...
    $tmp_mb_id = substr(
      preg_replace("/[^a-zA-Z0-9_]*/", "", $tmp_mb_id), 0, 20
    );
    // 최고관리자는 자동로그인 금지
    if (strtolower($tmp_mb_id) != strtolower($config['cf_admin'])) {
      $sql
        = " select mb_password, mb_intercept_date, mb_leave_date, mb_email_certify from {$g5['member_table']} where mb_id = '{$tmp_mb_id}' ";
      $row = sql_fetch($sql);
      if ($row['mb_password']) {
        $key = md5(
          $_SERVER['SERVER_ADDR'] . $_SERVER['SERVER_SOFTWARE']
          . $_SERVER['HTTP_USER_AGENT'] . $row['mb_password']
        );
        // 쿠키에 저장된 키와 같다면
...
if ($bo_table) {
  ...
  if ($board['bo_table']) {
    ...
    if (isset($wr_id) && $wr_id) {
      ...
      if (isset($wr_seo_title) && $wr_seo_title) {
        $write = get_content_by_field(
          $write_table, 'bbs', 'wr_seo_title', generate_seo_title($wr_seo_title)
        );
...
if (isset($config['cf_theme']) && trim($config['cf_theme'])) {
  ...
  if (is_dir($theme_path)) {
    ...
    define(
      'G5_THEME_URL', G5_URL . '/' . G5_THEME_DIR . '/' . $config['cf_theme']
    );
...
if (G5_IS_MOBILE) {
  ...
  $connect_skin_path = get_skin_path(
    'connect', $config['cf_mobile_connect_skin']
  );
  $connect_skin_url = get_skin_url(
    'connect', $config['cf_mobile_connect_skin']
  );
